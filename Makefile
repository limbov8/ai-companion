...
.PHONY: help init-env install install-audio-tools install-qwen-asr install-openvino-tts install-local-models check-docker db run run-no-db run-check run-check-no-db run-api run-api-local docker-run test gpu-status stop logs clean
...
help:
	...
	@echo "  make install-openvino-tts Install optional OpenVINO TTS backend"
...
install-openvino-tts:
	$(PYTHON) -m pip install -e ".[openvino-tts]"
